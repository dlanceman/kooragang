...
var_imp <- var_imp %>% 
  ...
  mutate(rank = 57 - rank(Importance),
         ...
         bandname = factor(bandname, levels = c("Elev", "NDVI", "Green", "Red", "NIR", "REG","NA")),
         ln.imp = log(Importance)) %>% 
  ungroup()
```

```{r visualise differences - plot band/variable importance}
# visualise differences in rank between bands and features ("variables")
# Plot bands
