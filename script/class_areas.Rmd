...
zero2 <- raster("raw/0mos_classified_1_scale.tif")
nine2 <- raster("raw/9mos_classified_1_scale.tif")
eighteen2 <- raster("raw/18mos_classified_1_scale.tif")
twentysix2 <- raster("raw/26mos_classified_1_scale.tif")
thirtyfour2 <- raster("raw/34mos_classified_1_scale.tif")
fortytwo2 <- raster("raw/42mos_classified_1_scale.tif")
fortysix2 <- raster("raw/46mos_classified_1_scale.tif")
...
plot(nine2)
plot(eighteen2)
plot(twentysix2) # background is labelled # 5 - need to remove
plot(thirtyfour2)
plot(fortytwo2)
plot(fortysix2) # background is labelled # 5 - need to remove
...
twsix2.crop <- crop(twentysix2, extent(boundary)) 
twsix2.crop <- mask(twsix2.crop, boundary)
plot(twsix2.crop)
...
fosix2.crop <- crop(fortysix2, extent(boundary)) 
fosix2.crop <- mask(fosix2.crop, boundary)
plot(fosix2.crop)
```
...
cells9DF$category <- c("na", "spor", "sarc", "grass", "mud", "water", "white_vege","road", "sarc_clumps") # add categories
cells9DF
...
cells18DF$category <- c("na", "spor","sarc", "grass", "mud", "water","sueda","road",  "white_vege") # add categories
cells18DF
...
cells26DF$category <- c("spor","sarc", "grass", "mud", "water","sueda", "road","white_vege", "na") # add categories
cells26DF
...
cells9DF1$category <- c("na", "spor", "sarc",  "grass", "mud", "water", "white_vege", "road", "sarc_clumps") # add categories
cells9DF1
...
# Calculating areas - 1m resolution
```{r 0 mos}
# calculate number of cells in each class
cells0 <- freq(zero2) # output is vectors
cells0DF2 <- as.data.frame(cells0) # make this a dataframe
cells0DF2$category <- c("na", "spor","sarc", "grass", "mud", "water", "white_vege","road") # add categories
cells0DF2

# determine cell size in map units
res(zero2) # cell size is 1 x 1 (metres)

# use this to calculate area of each class in hectares
cells0DF2$area <- cells0DF2$count*1*1/10000
cells0DF2

# spor is 3.24ha, sarc is 1.08ha (total is 4.32 ha) 
```
...
# 9 mos 
# calculate number of cells in each class
cells9 <- freq(nine2) # output is vectors
cells9DF2 <- as.data.frame(cells9) # make this a dataframe
cells9DF2$category <- c("na", "spor", "sarc", "grass", "mud", "water", "white_vege", "road",  "sarc_clumps") # add categories
cells9DF2

# determine cell size in map units
res(nine2) # cell size is 1 x 1 (metres)

# use this to calculate area of each class in hectares
cells9DF2$area <- cells9DF2$count*1*1/10000
cells9DF2

# spor = 3.36 ha, sarc = 0.82 ha, sarc clumps = 1.16 ha (total = 5.7 ha - similar to 0.5m resolution but different distribution between the classes)
...
# 18 mos
# calculate number of cells in each class
cells18 <- freq(eighteen2) # output is vectors
cells18DF2 <- as.data.frame(cells18) # make this a dataframe
cells18DF2$category <- c("na", "spor","sarc",  "grass", "mud", "water","sueda", "road","white_vege") # add categories
cells18DF2

# determine cell size in map units
res(eighteen2) # cell size is 1 x 1 (metres)

# use this to calculate area of each class in hectares
cells18DF2$area <- cells18DF1$count*1*1/10000
cells18DF2
...
# 26 mos 
# calculate number of cells in each class
cells26 <- freq(twsix2.crop) # output is vectors
cells26DF2 <- as.data.frame(cells26) # make this a dataframe
cells26DF2$category <- c("spor","sarc", "grass", "mud", "water","sueda", "road", "white_vege","na") # add categories
cells26DF2

# determine cell size in map units
res(twsix2.crop) # cell size is 1 x 1 (metres)

# use this to calculate area of each class in hectares
cells26DF2$area <- cells26DF2$count*1*1/10000
cells26DF2
...
# 34 mos 
# calculate number of cells in each class
cells34 <- freq(thirtyfour2) # output is vectors
cells34DF2 <- as.data.frame(cells34) # make this a dataframe
cells34DF2$category <- c("na", "spor","sarc","grass", "mud", "water","sueda","road", "white_vege") # add categories
cells34DF2

# determine cell size in map units
res(thirtyfour2) # cell size is 1 x 1 (metres)

# use this to calculate area of each class in hectares
cells34DF2$area <- cells34DF2$count*1*1/10000
cells34DF2
...
# 42 mos
# calculate number of cells in each class
cells42 <- freq(fortytwo2) # output is vectors
cells42DF2 <- as.data.frame(cells42) # make this a dataframe
cells42DF2$category <- c("na", "spor","sarc","grass", "mud", "water","algae","road", "white_vege", "sueda") # add categories
cells42DF2

# determine cell size in map units
res(fortytwo2) # cell size is 1 x 1 (metres)

# use this to calculate area of each class in hectares
cells42DF2$area <- cells42DF2$count*1*1/10000
cells42DF2
...
# 46 mos 
# calculate number of cells in each class
cells46 <- freq(fosix2.crop) # output is vectors
cells46DF2 <- as.data.frame(cells46) # make this a dataframe
cells46DF2$category <- c("spor","sarc","grass", "mud", "water","sueda","road", "white_vege", "na") # add categories
cells46DF2

# determine cell size in map units
res(fosix2.crop) # cell size is 1 x 1 (metres)

# use this to calculate area of each class in hectares
cells46DF2$area <- cells46DF2$count*1*1/10000
cells46DF2
...
areas %>% 
  filter(Scale == 0.1,
         category %in% c("spor", "sarc", "mud", "white_vege", "sueda")) %>%  
  ggplot(aes(x = date, y = area, color = category)) +
  geom_line() +
  theme_classic()
```
